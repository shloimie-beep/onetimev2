...
export function V21AppShell({
  ...
  return (
    <div className={`ot-v21 ot-v21--${role}`} lang="en">
      <a className="ot-v21__skip-link" href="#ot-v21-main">
      ...
      <header className="ot-v21__header">
        <a
          ...
          onClick={(event) => {
            ...
            onNavigate(navigation[0]?.href ?? '/');
          }}
          className="ot-v21__brand"
        >
          One Time
        </a>
...
export function V21Navigation({
  items,
  onNavigate,
}: {
  items: readonly V21NavigationItem[];
  onNavigate: (href: string) => void;
}) {
  return (
    <nav className="ot-v21__navigation" aria-label="Primary navigation">
      {items.map((item) =>
        item.disabledReason ? (
          <span
            key={item.id}
            className="ot-v21__nav-item ot-v21__nav-item--disabled"
            aria-disabled="true"
            title={disabledControlDescription(item.disabledReason)}
          >
            {item.label}
            <small>{disabledControlDescription(item.disabledReason)}</small>
          </span>
        ) : (
          <a
            key={item.id}
            className="ot-v21__nav-item"
            href={item.href}
...
export function V21StatePanel({
  ...
  return (
    <section
      className={`ot-v21__state ot-v21__state--${kind}`}
...
export function V21CalendarAgenda({
  items,
  selectedDate,
}: {
  items: readonly V21CalendarItem[];
  ...
  return (
    <section className="ot-v21__calendar" aria-labelledby="ot-v21-calendar-heading">
      <header>
        <h2 id="ot-v21-calendar-heading">Calendar</h2>
        <p aria-live="polite">
          {dateLabel}: {items.length} {items.length === 1 ? 'event' : 'events'}.
        </p>
      </header>
      <ol aria-label={`Agenda for ${dateLabel}`}>
        {items.map((item) => (
          <li key={item.id}>
            ...
            <span>
              {formatEnglishDate(item.startsAt, {
                month: 'short',
                weekday: undefined,
                year: undefined,
              })}
