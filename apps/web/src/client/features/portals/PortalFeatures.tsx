...
export function ParentPortalFeature({
  ...
  const selectedMaterials = selectedLearner
    ...
    : undefined;
  const selectedGamification = selectedLearner
...
export function StudentPortalFeature({
  ...
  return (
    ...
    >
      ...
      {dashboard.gamification.celebration && (
        <section className="ot-celebration" role="status" aria-live="polite">
          <strong>{dashboard.gamification.celebration.title}</strong>
          <span>{dashboard.gamification.celebration.detail}</span>
        </section>
      )}
      <StatusStrip viewState={viewState} onRetry={onRetry} />
