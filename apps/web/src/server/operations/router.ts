    ...
    protectedAdminOperation(ports, async (_request, response) => {
      ...
      sendAllowlistedJson(response, 200, {
        ...
        alerts: evaluateOperationsAlerts(result.snapshot).map((alert) => ({
          ...
          runtime_tier: alert.runtime_tier,
          verification_environment_id: alert.verification_environment_id,
...
function healthResponse(snapshot: OperationsHealthSnapshot) {
  return {
    ...
    queues: snapshot.queues.map((queue) => ({
      ...
      active_lease_count: queue.active_lease_count,
      unfenced_active_lease_count: queue.unfenced_active_lease_count,
      fencing_token_high_watermark: queue.fencing_token_high_watermark,
      retry_count: queue.retry_count,
      retry_scheduled_count: queue.retry_scheduled_count,
      retry_exhausted_count: queue.retry_exhausted_count,
      ...
      content_progress_age_ms: queue.content_progress_age_ms,
