...
function setup(input: { csrf?: boolean; assetRuntime?: boolean } = {}) {
  const descriptor = {
  ...
  const assetRuntime = {
    send: vi.fn(async (_req, res) => {
      res.status(200).type('video/mp4').send('private-bytes');
    }),
  };
  const app = express();
  ...
  app.use(
    ...
    createParentWelcomeRouter({
      service,
      sessions,
      ...(input.assetRuntime ? { assetRuntime } : {}),
      clock: () => new Date('2026-08-15T10:00:00.000Z'),
    }),
  );
  return { app, service, sessions, descriptor, assetRuntime };
}
...
describe('Parent welcome protected router', () => {
  ...
  it('authorizes the exact Parent again before delegating a same-origin asset', async () => {
    const { app, assetRuntime } = setup({ assetRuntime: true });
    const response = await send(app, '/api/app/parent/welcome-video/welcome-approved-v1/media', {
      headers: { cookie: 'ot_v21_parent=opaque', range: 'bytes=0-3' },
    });
    expect(response.status).toBe(200);
    expect(await response.text()).toBe('private-bytes');
    expect(assetRuntime.send).toHaveBeenCalledWith(
  ...
  it('fails closed before descriptor or asset resolution for missing and cross-household sessions', async () => {
    const missing = setup({ assetRuntime: true });
    missing.sessions.bootstrapCookieHeader.mockResolvedValueOnce({
      status: 'invalid',
    } as never);
    const unauthenticated = await send(
      missing.app,
      '/api/app/parent/welcome-video/welcome-approved-v1/media',
    );
    expect(unauthenticated.status).toBe(401);
    expect(missing.service.playbackDescriptor).not.toHaveBeenCalled();
    expect(missing.assetRuntime.send).not.toHaveBeenCalled();

    const crossHousehold = setup({ assetRuntime: true });
    crossHousehold.sessions.bootstrapCookieHeader.mockResolvedValueOnce({
      status: 'resolved',
      context: {
        ...context,
        session: { ...context.session, activeHouseholdId: 'other-household' },
      },
      csrf_token: 'csrf-parent',
      expires_at: '2026-08-15T12:00:00.000Z',
    } as never);
    ...
    expect(crossHousehold.service.playbackDescriptor).not.toHaveBeenCalled();
    expect(crossHousehold.assetRuntime.send).not.toHaveBeenCalled();
  });
  ...
  it('fails closed for media when no protected asset runtime is bound', async () => {
    ...
    expect(JSON.stringify(await response.json())).not.toMatch(/vimeo|drive|s3|https?:\/\//iu);
  });
