...
export function parseRunnerOptions(argv: string[], environment = process.env): RunnerOptions {
  ...
  return {
    rootPath: value('root') ?? environment.ONE_TIME_MEDIA_ROOT ?? path.join(environment.USERPROFILE ?? '.', 'OneTimeMedia'),
...
export function isStableFile(input: {
  ...
  return (
    ...
    (input.previousSize === null ||
      (input.previousSize === input.currentSize && input.previousModifiedAtMs === input.modifiedAtMs))
  );
}
...
export function buildVimeoReadyFfmpegArgs(input: { sourcePath: string; outputPath: string }) {
  ...
  return [
    '-hide_banner', '-nostdin', '-y', '-i', input.sourcePath,
    '-map', '0:v:0', '-map', '0:a?', '-vf', "scale='min(1920,iw)':'min(1080,ih)':force_original_aspect_ratio=decrease,fps=30",
...
async function loadSettings(rootPath: string): Promise<LocalMediaSettings> {
  ...
  const parsed = JSON.parse((await readFile(settingsPath, 'utf8')).replace(/^\uFEFF/, '')) as Partial<LocalMediaSettings>;
  const required = ['incomingDir', 'processingDir', 'readyForVimeoDir', 'completeDir', 'failedDir', 'stateDir', 'logsDir'] as const;
  for (const key of required) if (!parsed[key] || typeof parsed[key] !== 'string') throw new Error(`local_media_settings_${key}_required`);
  if (parsed.rootPath && path.resolve(parsed.rootPath) !== path.resolve(rootPath)) throw new Error('local_media_settings_root_mismatch');
  return {
    rootPath,
    incomingDir: parsed.incomingDir!, processingDir: parsed.processingDir!, readyForVimeoDir: parsed.readyForVimeoDir!,
    completeDir: parsed.completeDir!, failedDir: parsed.failedDir!, stateDir: parsed.stateDir!, logsDir: parsed.logsDir!,
...
function getJob(database: DatabaseSync, sourceSha256: string) {
  return database.prepare('SELECT * FROM local_media_jobs WHERE source_sha256 = ?').get(sourceSha256) as JobRow | undefined;
}

function saveJob(database: DatabaseSync, row: JobRow) {
  database.prepare(`INSERT INTO local_media_jobs
    ...
    .run(row.source_sha256, row.source_path, row.display_name, row.state, row.source_size, row.source_mtime_ms, row.output_path, row.output_sha256, row.detail_code, row.updated_at);
}

async function checkFfprobe(filePath: string) {
  await runCommand('ffprobe', ['-v', 'error', '-show_format', '-show_streams', '-of', 'json', filePath]);
}
...
async function processOneLocal(settings: LocalMediaSettings, database: DatabaseSync, row: JobRow) {
  ...
  saveJob(database, { ...row, state: 'processing', detail_code: 'local_ffmpeg_in_progress', updated_at: new Date().toISOString() });
  try {
    await runCommand('ffmpeg', buildVimeoReadyFfmpegArgs({ sourcePath: row.source_path, outputPath: processingPath }));
    ...
    saveJob(database, { ...row, state: 'processed', output_path: readyPath, output_sha256: outputSha256, detail_code: 'review_required_before_any_provider_effect', updated_at: new Date().toISOString() });
  } catch (error) {
    saveJob(database, { ...row, state: 'failed', detail_code: safeErrorCode(error), updated_at: new Date().toISOString() });
  }
}

async function writeReviewManifest(settings: LocalMediaSettings, row: JobRow, readyPath: string, outputSha256: string) {
  const manifest = {
    schema_version: 1,
    source_sha256: row.source_sha256,
    ...
    state: 'processed',
    vimeo_ready: true,
    transcription: settings.transcriptionMode === 'off' ? 'disabled_by_settings' : 'not_executed_requires_explicit_acceptance',
    library_import: 'not_executed_requires_explicit_acceptance',
    provider_effects_executed: false,
    raw_paths_present: false,
    required_review: ['operator confirms the derivative', 'operator selects the exact class occurrence', 'operator explicitly accepts one private Vimeo upload'],
    retention: { raw_source_days_minimum: Math.max(settings.rawSourceRetentionDays, 7), deletion_authorized: false },
  };
...
async function scanOnce(settings: LocalMediaSettings, processLocal: boolean) {
  ...
  try {
    const entries = await (await import('node:fs/promises')).readdir(settings.incomingDir, { withFileTypes: true });
    for (const entry of entries) {
      ...
      const stable = isStableFile({ modifiedAtMs: sourceStat.mtimeMs, nowMs: Date.now(), stableFileSeconds: settings.stableFileSeconds, previousSize: previous?.source_size ?? null, currentSize: sourceStat.size, previousModifiedAtMs: previous?.source_mtime_ms ?? null });
      const row: JobRow = {
        source_sha256: sourceSha256, source_path: sourcePath, display_name: entry.name,
        state: stable ? 'ready_for_processing' : 'waiting_for_stability', source_size: sourceStat.size,
        source_mtime_ms: sourceStat.mtimeMs, output_path: previous?.output_path ?? null, output_sha256: previous?.output_sha256 ?? null,
        detail_code: stable ? 'local_review_queue' : 'awaiting_60_second_stability_window', updated_at: new Date().toISOString(),
      };
      saveJob(database, row);
      if (stable) {
        try { await checkFfprobe(sourcePath); } catch (error) { saveJob(database, { ...row, state: 'failed', detail_code: safeErrorCode(error), updated_at: new Date().toISOString() }); continue; }
...
async function showStatus(settings: LocalMediaSettings) {
  const database = openState(settings);
  try {
    const jobs = database.prepare('SELECT * FROM local_media_jobs ORDER BY updated_at DESC').all() as JobRow[];
    process.stdout.write(`${JSON.stringify({ runner: 'local_only', provider_effects_executed: false, jobs: jobs.map(sanitizeJob) }, null, 2)}\n`);
  } finally { database.close(); }
}
...
async function retry(settings: LocalMediaSettings, sourceSha256: string) {
  ...
  try {
    ...
    saveJob(database, { ...row, state: 'retry_wait', detail_code: 'operator_requested_local_retry', updated_at: new Date().toISOString() });
  } finally { database.close(); }
}

async function validateInstallation(settings: LocalMediaSettings) {
  for (const directory of [settings.rootPath, settings.incomingDir, settings.processingDir, settings.readyForVimeoDir, settings.stateDir, settings.logsDir]) {
...
async function registerCurrentUserTask(settings: LocalMediaSettings) {
  ...
  await runCommand('schtasks.exe', ['/Create', '/TN', 'One Time Local Media Runner', '/TR', wrapperPath, '/SC', 'ONLOGON', '/RL', 'LIMITED', '/F']);
}
...
function runCommand(command: string, args: string[]) {
  return new Promise<void>((resolve, reject) => {
    ...
    child.on('close', (code) => code === 0 ? resolve() : reject(new Error(`${command}_exit_${code}:${Buffer.concat(errors).toString('utf8').slice(-180)}`)));
  });
}

function safeErrorCode(error: unknown) {
  return (error instanceof Error ? error.message : String(error)).replace(/[^a-zA-Z0-9_:-]+/g, '_').slice(0, 160) || 'local_media_error';
}
...
async function main() {
  ...
  if (command === 'retry') { if (!options.retrySourceSha256) throw new Error('retry_source_sha256_required'); return retry(settings, options.retrySourceSha256); }
  if (command === 'install-windows') {
    ...
    process.stdout.write(`${JSON.stringify({ installation: 'validated', task_scheduler: options.applyTaskScheduler ? 'registered_current_user' : 'not_registered_without_explicit_apply', provider_effects_executed: false })}\n`);
    return;
  }
  if (command === 'uninstall-windows') {
    if (options.applyTaskScheduler) await unregisterCurrentUserTask();
    process.stdout.write(`${JSON.stringify({ uninstall: options.applyTaskScheduler ? 'current_user_task_removed_media_preserved' : 'no_action_without_explicit_apply', provider_effects_executed: false })}\n`);
    return;
  }
  if (command !== 'start') throw new Error('local_media_runner_unknown_command');
  do { await scanOnce(settings, options.processLocal); if (!options.once) await new Promise((resolve) => setTimeout(resolve, 15_000)); } while (!options.once);
}

if (import.meta.main) void main().catch((error) => { process.stderr.write(`${safeErrorCode(error)}\n`); process.exit(1); });
