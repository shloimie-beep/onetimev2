# V43 — Operations, Domain, Migration, Recovery, and Retired-Surface Verification — START OR RESUME
...
Task packet: `ops/v2.1-execution/tasks/V43.yaml`  
Locked context: `ops/v2.1-execution/contexts/V43-CONTEXT.md`  
Task state: `ops/v2.1-execution/runtime/V43/TASK-STATE.yaml`  
Handoff: `ops/v2.1-execution/runtime/V43/HANDOFF.md`  
Next prompt: `ops/v2.1-execution/runtime/V43/NEXT-PROMPT.md`
...
2. Read the exact V43 status/branch from
...
Verify runtime identity, migrations, queues, monitoring, backup/restore/rollback mechanisms, canary/legal gates, domains, migration bridge, archival behavior, and retired-surface absence.

Verification source tasks: `P33, P34, P35`  
Assigned requirements: `19`  
Assigned acceptance cases: `20`  
Evidence path:
`ops/v2.1-execution/results/<candidate-digest>/V43/<case-id>/attempt-<n>.yaml`

## Start or resume

1. Fetch remote refs and read the control-authorized immutable candidate manifest. Refuse a moving or ambiguous candidate.
2. Read V43's exact branch/status and evidence target from the fetched
   remote control registry/ready queue. Create/resume
   `codex/v21-verify-<candidate-short-sha>-v43` from the exact
...
4. Read this lane context, source-task handoffs,
   `runtime/V43/TASK-STATE.yaml`, `HANDOFF.md`, relevant named code, and
...
8. On an ordinary harness/environment issue within lane scope, diagnose and
   ...
   `ops/v2.1-execution/verification-harness/V43/` on the separately
   registered sibling branch
   `codex/v21-harness-<candidate-short-sha>-v43`; that path is
...
Continue until all 20 assigned cases have candidate-bound `passed`, `failed`, or precise `blocked` records and every independent case has been attempted. Release requires all to pass; this lane must not convert a failure/blocker into a waiver.
...
## V43 two-phase verification

Phase A runs with the other verifier lanes. Execute and pass every assigned case
whose preconditions do not require the R44 production operator journey. There
are exactly three deferred cases:
`OTV2-OPS-219-AC01`, `OTV2-OPS-241-ENVIRONMENT-MAPPING`, and
`OTV2-OPS-241-LEGAL-ARTIFACT-GATE`. Record each as `blocked` with
`blocker_code: pending_R44`, never waived or passed. C00 may authorize R44 only
when the other 17 V43 cases are passed and their partial V43 index is
aggregated.

After R44 evidence is aggregated and P34 Phase B's non-result supporting proof
is aggregated at
`proofs/<candidate-digest>/P34/POST-OPERATOR-PROOF.yaml`, C00 issues a
`resume_ready` lease and records `active_phase: phase_b_postoperator`,
the exact V43 candidate branch, expected Phase A head, and current evidence head
under V43's `phase_branches`/`phase_heads`. Resume that registered branch from
the current evidence head only when it descends from the expected Phase A head;
fast-forward with `--ff-only`. Validate the proof without treating it as a pass, execute the
three deferred cases, and write V43-owned immutable attempts that supersede
V43's own `pending_R44` records. Rebuild the V43 lane summary and push. I36 must
aggregate this Phase B head before R45. Any product-code change requires a new
candidate and invalidates the verification set.
