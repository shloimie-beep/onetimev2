# P34 — Backup, Restore, Rollback, Canary Budgets, and Legal Gate — START OR RESUME
...
Implement backup/restore proof, rollback controls, bounded canary accounting, and a non-fabricatable legal-policy artifact gate; close the post-operator legal/canary evidence after R44.

Repository: `shloimie-beep/onetimev2`  
Task packet: `ops/v2.1-execution/tasks/P34.yaml`  
Locked context: `ops/v2.1-execution/contexts/P34-CONTEXT.md`  
Runtime state: `ops/v2.1-execution/runtime/P34/TASK-STATE.yaml`  
...
3. Read the exact branch/status for P34 from fetched
...
5. If it does not exist, read `READY-QUEUE.yaml` from the fetched remote control
   branch. Start only when P34 is `ready`, its locks are unclaimed, and the
...
Primary requirements (6): `OTV2-OPS-168, OTV2-OPS-169, OTV2-OPS-170, OTV2-OPS-218, OTV2-OPS-219, OTV2-OPS-241`  
Primary acceptance cases (7): `OTV2-OPS-168-AC01, OTV2-OPS-169-AC01, OTV2-OPS-170-AC01, OTV2-OPS-218-AC01, OTV2-OPS-219-AC01, OTV2-OPS-241-ENVIRONMENT-MAPPING, OTV2-OPS-241-LEGAL-ARTIFACT-GATE`  
Writer slots: `OPERATIONS_RECOVERY`  
Start after: `P33`  
Full merge after: `P33`  
Candidate integration partners (non-ordering): `P35`  
Acceptance after: `R44`
...
Phase A is normal implementation on the P34 task branch: backup/restore,
rollback, canary-budget, and legal-gate mechanisms become `ready_for_review` and
are integrated before candidate freeze.

Phase B begins only after V37–V43 evidence and R44 operator evidence have been
aggregated. C00 must register `active_phase: phase_b_supporting_proof`, the
candidate-specific branch
`codex/v21-p34-evidence-<candidate-short-sha>`, and its exact start/head under
P34's `phase_branches`/`phase_heads`; a fresh window reads those fields. Do not
change product code. Create/resume that exact branch from the current candidate
evidence head and produce only the schema-valid supporting proof at
`ops/v2.1-execution/proofs/<candidate-digest>/P34/POST-OPERATOR-PROOF.yaml`
for `OTV2-OPS-219-AC01`, `OTV2-OPS-241-ENVIRONMENT-MAPPING`, and
`OTV2-OPS-241-LEGAL-ARTIFACT-GATE`, along with P34 state/handoff files. This
proof binds R44/evidence/candidate/environment/legal/recovery digests but is not
an acceptance result and must not claim those cases passed. Push and notify C00
so I36 can aggregate the supporting proof. C00 then reauthorizes V43 Phase B;
V43 alone writes/supersedes the three acceptance results. Any required
product-code change invalidates the candidate and returns to
implementation/integration; it may not be hidden in Phase B.
