# V37 — Identity, Public Entry, and Admin Verification — START OR RESUME
...
Task packet: `ops/v2.1-execution/tasks/V37.yaml`  
Locked context: `ops/v2.1-execution/contexts/V37-CONTEXT.md`  
Task state: `ops/v2.1-execution/runtime/V37/TASK-STATE.yaml`  
Handoff: `ops/v2.1-execution/runtime/V37/HANDOFF.md`  
Next prompt: `ops/v2.1-execution/runtime/V37/NEXT-PROMPT.md`
...
2. Read the exact V37 status/branch from
...
Verify authority convergence, retired-surface absence, adult/Student authentication, identity/tenant isolation, Family/School entry, and Admin directory/search against the immutable candidate.

Verification source tasks: `F01, F03, F04, P08, P09, P10, P11`  
Assigned requirements: `48`  
Assigned acceptance cases: `55`  
Evidence path:
`ops/v2.1-execution/results/<candidate-digest>/V37/<case-id>/attempt-<n>.yaml`

## Start or resume

1. Fetch remote refs and read the control-authorized immutable candidate manifest. Refuse a moving or ambiguous candidate.
2. Read V37's exact branch/status and evidence target from the fetched
   remote control registry/ready queue. Create/resume
   `codex/v21-verify-<candidate-short-sha>-v37` from the exact
...
4. Read this lane context, source-task handoffs,
   `runtime/V37/TASK-STATE.yaml`, `HANDOFF.md`, relevant named code, and
...
8. On an ordinary harness/environment issue within lane scope, diagnose and
   ...
   `ops/v2.1-execution/verification-harness/V37/` on the separately
   registered sibling branch
   `codex/v21-harness-<candidate-short-sha>-v37`; that path is
...
Continue until all 55 assigned cases have candidate-bound `passed`, `failed`, or precise `blocked` records and every independent case has been attempted. Release requires all to pass; this lane must not convert a failure/blocker into a waiver.
