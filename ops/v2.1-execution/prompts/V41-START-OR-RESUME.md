# V41 — Provider Foundation, Billing, Access, and GHL Identity Verification — START OR RESUME
...
Locked context: `ops/v2.1-execution/contexts/V41-CONTEXT.md`  
Task state: `ops/v2.1-execution/runtime/V41/TASK-STATE.yaml`  
Handoff: `ops/v2.1-execution/runtime/V41/HANDOFF.md`  
Next prompt: `ops/v2.1-execution/runtime/V41/NEXT-PROMPT.md`
...
2. Read the exact V41 status/branch from
...
Verify provider idempotency/reconciliation, commercial billing flows, signed access projection, grace/recovery, adult GHL mapping, and zero Student GHL contacts.

Verification source tasks: `F06, P25, P26, P27`  
Assigned requirements: `27`  
Assigned acceptance cases: `29`  
Evidence path:
`ops/v2.1-execution/results/<candidate-digest>/V41/<case-id>/attempt-<n>.yaml`

## Start or resume

1. Fetch remote refs and read the control-authorized immutable candidate manifest. Refuse a moving or ambiguous candidate.
2. Read V41's exact branch/status and evidence target from the fetched
   remote control registry/ready queue. Create/resume
   `codex/v21-verify-<candidate-short-sha>-v41` from the exact
...
4. Read this lane context, source-task handoffs,
   `runtime/V41/TASK-STATE.yaml`, `HANDOFF.md`, relevant named code, and
...
8. On an ordinary harness/environment issue within lane scope, diagnose and
   ...
   `ops/v2.1-execution/verification-harness/V41/` on the separately
   registered sibling branch
   `codex/v21-harness-<candidate-short-sha>-v41`; that path is
...
Continue until all 29 assigned cases have candidate-bound `passed`, `failed`, or precise `blocked` records and every independent case has been attempted. Release requires all to pass; this lane must not convert a failure/blocker into a waiver.
