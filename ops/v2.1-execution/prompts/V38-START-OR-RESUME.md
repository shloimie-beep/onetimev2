# V38 — UX, Parent, Student, Calendar, Support, and Copy Verification — START OR RESUME
...
Locked context: `ops/v2.1-execution/contexts/V38-CONTEXT.md`  
Task state: `ops/v2.1-execution/runtime/V38/TASK-STATE.yaml`  
Handoff: `ops/v2.1-execution/runtime/V38/HANDOFF.md`  
Next prompt: `ops/v2.1-execution/runtime/V38/NEXT-PROMPT.md`
...
2. Read the exact V38 status/branch from
...
Verify role journeys, responsive/accessibility behavior, Parent/Student privacy boundaries, calendars, support, and exact approved copy against the immutable candidate.

Verification source tasks: `F07, P12, P13, P14, P15, P24, P31`  
Assigned requirements: `58`  
Assigned acceptance cases: `58`  
Evidence path:
`ops/v2.1-execution/results/<candidate-digest>/V38/<case-id>/attempt-<n>.yaml`

## Start or resume

1. Fetch remote refs and read the control-authorized immutable candidate manifest. Refuse a moving or ambiguous candidate.
2. Read V38's exact branch/status and evidence target from the fetched
   remote control registry/ready queue. Create/resume
   `codex/v21-verify-<candidate-short-sha>-v38` from the exact
...
4. Read this lane context, source-task handoffs,
   `runtime/V38/TASK-STATE.yaml`, `HANDOFF.md`, relevant named code, and
...
8. On an ordinary harness/environment issue within lane scope, diagnose and
   ...
   `ops/v2.1-execution/verification-harness/V38/` on the separately
   registered sibling branch
   `codex/v21-harness-<candidate-short-sha>-v38`; that path is
...
Continue until all 58 assigned cases have candidate-bound `passed`, `failed`, or precise `blocked` records and every independent case has been attempted. Release requires all to pass; this lane must not convert a failure/blocker into a waiver.
