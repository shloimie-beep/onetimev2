# V39 — Schema, Jobs, Classroom, Zoom, and Content Verification — START OR RESUME
...
Task packet: `ops/v2.1-execution/tasks/V39.yaml`  
Locked context: `ops/v2.1-execution/contexts/V39-CONTEXT.md`  
Task state: `ops/v2.1-execution/runtime/V39/TASK-STATE.yaml`  
Handoff: `ops/v2.1-execution/runtime/V39/HANDOFF.md`  
Next prompt: `ops/v2.1-execution/runtime/V39/NEXT-PROMPT.md`
...
2. Read the exact V39 status/branch from
...
Verify schema/state invariants, jobs, canonical enrollment, embedded Zoom, attendance, upload/processing/publication/playback, and denial paths against the immutable candidate.

Verification source tasks: `F02, F05, P16, P17, P18, P19, P20, P21`  
Assigned requirements: `46`  
Assigned acceptance cases: `49`  
Evidence path:
`ops/v2.1-execution/results/<candidate-digest>/V39/<case-id>/attempt-<n>.yaml`

## Start or resume

1. Fetch remote refs and read the control-authorized immutable candidate manifest. Refuse a moving or ambiguous candidate.
2. Read V39's exact branch/status and evidence target from the fetched
   remote control registry/ready queue. Create/resume
   `codex/v21-verify-<candidate-short-sha>-v39` from the exact
...
4. Read this lane context, source-task handoffs,
   `runtime/V39/TASK-STATE.yaml`, `HANDOFF.md`, relevant named code, and
...
8. On an ordinary harness/environment issue within lane scope, diagnose and
   ...
   `ops/v2.1-execution/verification-harness/V39/` on the separately
   registered sibling branch
   `codex/v21-harness-<candidate-short-sha>-v39`; that path is
...
Continue until all 49 assigned cases have candidate-bound `passed`, `failed`, or precise `blocked` records and every independent case has been attempted. Release requires all to pass; this lane must not convert a failure/blocker into a waiver.
