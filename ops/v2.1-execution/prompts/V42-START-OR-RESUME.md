# V42 — GHL Communications and Workflow Verification — START OR RESUME
...
Task packet: `ops/v2.1-execution/tasks/V42.yaml`  
Locked context: `ops/v2.1-execution/contexts/V42-CONTEXT.md`  
Task state: `ops/v2.1-execution/runtime/V42/TASK-STATE.yaml`  
Handoff: `ops/v2.1-execution/runtime/V42/HANDOFF.md`  
Next prompt: `ops/v2.1-execution/runtime/V42/NEXT-PROMPT.md`
...
2. Read the exact V42 status/branch from
...
Verify sender/suppression/readback, website bot, email-first WhatsApp-dormant behavior, lifecycle workflows, newsletter, reactivation, and conversion definitions/readbacks.

Verification source tasks: `P28, P29, P30`  
Assigned requirements: `24`  
Assigned acceptance cases: `28`  
Evidence path:
`ops/v2.1-execution/results/<candidate-digest>/V42/<case-id>/attempt-<n>.yaml`

## Start or resume

1. Fetch remote refs and read the control-authorized immutable candidate manifest. Refuse a moving or ambiguous candidate.
2. Read V42's exact branch/status and evidence target from the fetched
   remote control registry/ready queue. Create/resume
   `codex/v21-verify-<candidate-short-sha>-v42` from the exact
...
4. Read this lane context, source-task handoffs,
   `runtime/V42/TASK-STATE.yaml`, `HANDOFF.md`, relevant named code, and
...
8. On an ordinary harness/environment issue within lane scope, diagnose and
   ...
   `ops/v2.1-execution/verification-harness/V42/` on the separately
   registered sibling branch
   `codex/v21-harness-<candidate-short-sha>-v42`; that path is
...
Continue until all 28 assigned cases have candidate-bound `passed`, `failed`, or precise `blocked` records and every independent case has been attempted. Release requires all to pass; this lane must not convert a failure/blocker into a waiver.
