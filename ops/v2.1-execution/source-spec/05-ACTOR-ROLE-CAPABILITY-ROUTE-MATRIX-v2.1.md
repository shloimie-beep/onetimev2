...
Before `2026-09-11T18:00:00+03:00`, One Time Family signup collects only adult first name, adult last name, email, password, editable IANA timezone, and one required initially unchecked agreement control. That one click records `one_time_family_signup_unified_v1`, UTC timestamp, adult Terms/Privacy/related agreement acceptance, and adult marketing/newsletter email consent as `opted_in`; no separate marketing/newsletter control appears. A committed local Family transaction creates immediate free access, then alone emits the adult-only GHL event using One Time Household ID as its reconciliation/idempotency key, directly maps the opportunity to Active Member, adds only `ot | lead` and `ot | email opt-in`, and enrolls OT-01. It never transmits password/security data, creates a Student GHL contact, or authorizes SMS, automated calls, or WhatsApp. At or after that instant, the same public route creates the account and inactive household, then requires standard hosted checkout before Student creation or learning access. It never creates a rolling free trial.
