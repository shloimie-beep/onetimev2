...
PR #133 at exact head `ff8aed26c6a21bf525d18a493e163a2af1288ad5` is historical evidence. Its package numbers must not be imported literally because current PR #183 at exact head `595e38b2c53c7f9dd1b0018ae1954e42061f07f4` assigns those same numbers to different hooks.
...
This packet therefore creates only current `OTM-CP-001` and `OTM-CP-002` draft derivatives plus the shared text-free landing handoff explicitly required by the current media work order. It does not merge PR #133 wholesale, overwrite or delete any legacy asset, write to its Drive folders, replay any provider effect, or claim the legacy outputs are current final creatives.
