...
Every derivative uses the approved Rabbi-only `Class Photo - Rabbi Eli Teaching - Vertical 01.jpeg`. The six social drafts also use the text-free One Time white logo. Their logos, foreground copy, CTA treatments, rules, and URLs remain at least 7% inside every applicable outer edge. Bounds are captured from the actual Pillow logo alpha, text glyph, CTA, and rectangle operations; no handwritten foreground union is accepted. No Student image, generated person, face modification, source-file move/rename, Drive write, Canva write, product write, provider action, scheduling, publication, broadcast, boost, or spend occurred.
