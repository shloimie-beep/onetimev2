...
| `03-OT-GHL`        | `Q03-003` | `PRODUCT DECISION REQUIRED`, then `APPROVE SEND` | First accept the unblocked repository repair `Q03-002R`; PR #126 alone does not unlock provider work. Then explicitly decide Rabbi Phase-2 mailbox/routing/Reply-To ownership and prove HighLevel From acceptance; after that, separately authorize one exact operator-owned seed and one reply readback. |
...
| `07-OT-PRODUCTION` | `Q07-001` | `BLOCKED`                                        | Production approval is denied because Q07-001 is not ready. Do not request approval again until an immutable candidate, accepted backup/restore/rollback proof, every named dependency, and fresh exact Board authority are all present.                                                                  |
