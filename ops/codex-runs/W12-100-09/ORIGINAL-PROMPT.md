# W12-100-09 Original Prompt

Captured: 2026-07-17T18:01:32+03:00
...
- apps/web/src/server/features/billing/**
- billing-specific contracts, scripts, and tests
- ops/codex-runs/W12-100-09/**

This lane is Stripe TEST mode only. Live Stripe actions are forbidden.

Audit:
- product, price, amount, currency, account, product-scope, and principal guards
- checkout creation
...
- test/live object separation
- reconciliation dry-run
- authorization for reconciliation apply
- redacted provider errors
- no secret or full customer object logging

Use fake Stripe fixtures and the official test adapter without making an
external API call. Add tests proving every live-mode object is rejected.

Produce a launch decision matrix:
A. launch free with billing disabled
B. launch with Stripe TEST operator validation only
C. later paid-production launch

Do not choose C unless all missing live-mode controls and business decisions are
explicitly documented. No test resource setup or external Stripe canary may run
in this lane.
```
