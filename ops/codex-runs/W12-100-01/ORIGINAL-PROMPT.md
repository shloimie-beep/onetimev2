...
5. External actions and production mutations must both be zero.
   ...
   - ops/codex-runs/W12-100-01/**

   This is primarily a test-and-audit lane. Do not edit packages/config/src/index.ts,
   apps/worker/src/delivery/**, migrations, or deployment records.

   Perform a threat-model-driven audit of:
   - account and product scoping
   - owner/admin, parent, and student authorization boundaries
   - session creation, rotation, expiration, and revocation
   - cookie flags and trusted-device behavior
   - login timing resistance and durable rate limits
   - owner/admin email assurance and MFA
   - CSRF on every state-changing browser route
   - safe return_to handling and open-redirect resistance
   - activation/reset token storage, expiry, replay, and invalidation
   - webhook signature verification, replay rejection, body limits, and dedupe
   - provider URL handling and SSRF risk
   - SQL parameterization and dynamic identifier handling
   - sensitive-data logging and error redaction
   - CSP and security headers
   - cross-household and sibling isolation
   - production prohibition of the Portal Test Lab
   - BNA boundary enforcement

   Create a concise threat model with:
   asset, actor, trust boundary, attack path, current control, residual risk,
   severity, evidence, and proposed remediation.

   Add adversarial automated tests for high-risk boundaries. Prefer tests that
   demonstrate the existing control before proposing source changes.

   Do not make broad source edits. A narrowly proven security bug may be fixed only
   inside its directly owned module and tests. Any required changes to app.ts,
   config, migrations, or worker delivery must be recorded as a convergence
   dependency.

   Include a release-blocking findings section. A blocker must have reproducible
