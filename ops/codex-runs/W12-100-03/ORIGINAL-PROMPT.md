# W12-100-03 Original Prompt

Received for this lane on 2026-07-17.
...
5. External actions and production mutations must both be zero.
   ...
   Exclusive ownership:
   - packages/domain/src/landing/**
   - new packages/domain/src/legal/**
   - scripts/build-public-pages.ts
   - packages/brand-system public-page styles required by this lane
   - public-page E2E/accessibility tests
   - ops/codex-runs/W12-100-03/**

   The current privacy and terms pages are placeholders. Build launch-quality,
   counsel-review-ready public documents without inventing legal compliance claims.

   Create structured, versioned content for:
   - Privacy Notice
   - Terms of Use
   - Communication and Reminder Consent
   - Parent/Guardian and Student Data Notice
   - Cancellation/payment language only to the extent billing is actually enabled
   - contact and policy-version metadata

   The privacy notice must truthfully enumerate the data categories visible in the
   current system, including signup, account, household, learner, class, progress,
   communications, support, provider event, payment/test-payment, security, and
   operational records. Describe purpose and handling at a high level without
   publishing internal architecture or retention claims that have not been
   approved.

   The terms must not continue saying that accounts are never granted if the
   system now has owner, parent, and student accounts. Do not claim paid checkout
   is live unless current capability evidence proves it. Mark decisions needing
   business or legal approval explicitly in the run report rather than guessing.

   Improve signup consent so:
   - email and WhatsApp permissions are channel-specific
   - no consent is inferred from a preselected channel
   - required service communications are distinguished from optional reminders
   - suppression/STOP/unsubscribe behavior is clear
   - policy version and consent timestamp can be recorded
   - the public form does not ask for student-sensitive data

   Add keyboard, screen-reader, mobile, validation, and no-JavaScript checks.
   Preserve public/authenticated bundle separation and current brand constraints.
```
