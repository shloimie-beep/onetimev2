...
Primary ownership:
- apps/telegram-bot/**
- packages/domain/src/telegram/**
- packages/domain/src/whatsapp/**
- Telegram/WhatsApp contracts
- messaging-specific tests
- ops/codex-runs/W12-100-08/**

Do not edit shared config or the delivery worker. Record any required changes to
those files as dependencies on W12-100-02.

For Telegram verify:
- one active consumer model
- webhook/polling mutual exclusion
- webhook secret validation
- bounded body depth, size, and array/string limits
- encrypted durable inbox
- event dedupe and replay protection
- operator/chat allowlist
- capability and role enforcement
- redacted contact/support/question output
- preview/confirmation digest for writes
- confirmation expiry and single use
- delivery retry cannot broaden account/product scope
- no raw token or chat ID in evidence

For WhatsApp verify:
- challenge and signature validation
- replay, duplicate, malformed, and oversized webhook handling
- sender/account rate limits
- opt-out and suppression precedence
- consent before outbound eligibility
- public assistant offline/readiness truth
- deep-link and prefill validation
- no browser persistence of private state
- account/product scope
- one-recipient canary budget enforcement

Use local fake provider endpoints only. Produce separate Telegram and WhatsApp
staging readiness matrices with every required protected value represented only
as present, absent, valid, or invalid.
