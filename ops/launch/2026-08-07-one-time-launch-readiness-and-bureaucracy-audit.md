...
- **Completion proof:** stages saved/reopened in order; `b87...` read back as `Active Member`; new funnels enter `Warm Leads`; source fields populated; no Student contact; one direct Tisha email receipt; one direct old-member migration email receipt; `OT-02A` inactive; no `OT-02C`.
