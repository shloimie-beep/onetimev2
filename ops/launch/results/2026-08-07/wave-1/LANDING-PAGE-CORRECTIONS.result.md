...
- Focused runtime public-metadata suite — 3 passed.
- Repository secret-pattern scan — passed across 3,381 text files with zero
  findings.
...
- Normalized the three files identified by the child PR's formatting check.
- Prevented a teaching-photo swipe from also advancing again through its
  synthetic click, and added the same direction-aware swipe behavior to the
  press carousel.
- Kept the legacy gallery controls inert and out of keyboard/screen-reader
  focus while preserving the visible carousel's direct keyboard controls.
- Gave the teaching-location carousel its own heading instead of repeating the
  press-section heading.
- Recaptured all five responsive screenshots from the integrated build with
  images loaded and reduced motion enabled.
