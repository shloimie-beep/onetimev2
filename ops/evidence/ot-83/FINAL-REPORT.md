...
- Added narrow Prettier ignores for repo-backed Codex provenance packets so
  CI does not rewrite verbatim prompt/source evidence.
...
- `npm run format`: failed locally on Windows because `core.autocrlf=true`
  checks out inherited LF-index files as CRLF; `.prettierignore` was narrowed
  to preserve Codex run packet provenance that failed Linux CI formatting.
