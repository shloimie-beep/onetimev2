...
This branch adds a merge-ready CRM module surface behind explicit integration hooks. It does not wire the module into the live app shell, mutate production data, or edit the prohibited central server and application files.

The current OT-39 app already owns canonical `/app/crm` and `/api/v1/crm` routes. OT-42 adds a CRM-owned router/register module that an integration lane can mount later without modifying this branch.
