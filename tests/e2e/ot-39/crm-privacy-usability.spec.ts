...
test('static and runtime checks prevent GET free-text search', async ({ page }) => {
  ...
  expect(adapterSource).toContain('if (query.search.trim()) throw new SearchUnavailableError()');
  ...
  await expect(page.getByText('Free-text contact search is paused')).toBeVisible();
  ...
  const browserLeakState = await page.evaluate((needle) => {
    ...
    return {
      ...
      containsNeedle:
        ...
        [...document.querySelectorAll<HTMLInputElement>('input')].some((input) =>
          input.value.includes(needle),
        ) ||
...
test('rows open by keyboard and Back restores focus from cached list without refetch', async ({
  ...
  expect(requestPaths.filter((requestPath) => requestPath === '/api/v1/crm/contacts')).toEqual([]);
