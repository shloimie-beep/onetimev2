...
describe('OT-P0 canonical operator role reconciliation', () => {
  it('adds only the missing Parent membership and remains idempotent', async () => {
    ...
    await pool.query(
      `INSERT INTO onetime.v21_adult_identities
       ...
       VALUES ('adult_operator', 'sdratler@gmail.com', 'Operator Adult', 'active', 1,
               'one_time_mishnayos', 'production', 'production_operator_canary', $1, $1)`,
      [occurredAt],
    );
    await pool.query(
    ...
    await pool.query(
      `INSERT INTO onetime.v21_human_account_role_memberships
       ...
       VALUES ('account_operator', 'admin', $1, 'existing_admin_membership',
               'one_time_mishnayos', 'production', 'production_operator_canary')`,
      [occurredAt],
    );
    await pool.query(
      `INSERT INTO onetime.v21_adult_credentials
       ...
       VALUES ('account_operator', 'adult_operator', 'argon2id$existing-credential-hash',
               'active', 4, 'one_time_mishnayos', 'production',
               'production_operator_canary', $1, $1)`,
      [occurredAt],
    );
    await pool.query(
      `INSERT INTO onetime.v21_households
       ...
       VALUES ('household_operator', 'adult_operator', 'account_operator', 'family',
               ...
               'production', 'production_operator_canary', $1, $1)`,
      [occurredAt],
    );
    ...
    const migration = await readFile(
      path.resolve(
        process.cwd(),
        'packages/db/migrations/2274_operator_dual_role_reconciliation.sql',
      ),
      'utf8',
    );
