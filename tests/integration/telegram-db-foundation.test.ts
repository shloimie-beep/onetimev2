...
describe('OT-51P durable PostgreSQL contract through pg-mem', () => {
  it('applies all migrations and documents pg-mem no-op rerun limitation', async () => {
    ...
    expect(first.some((migration) => migration.id === '2010_ops06_reliability_observability')).toBe(
      true,
    );
    expect(
      first.some((migration) => migration.id === '2003_ot89a_subscriber_support_producer'),
    ).toBe(true);
    expect(first.at(-1)?.id).toBe('2010_ops06_reliability_observability');
    const applied = await pool.query(
      `SELECT checksum
        ...
        WHERE id IN (
          ...
          '2010_ops06_reliability_observability'
        )`,
    );
