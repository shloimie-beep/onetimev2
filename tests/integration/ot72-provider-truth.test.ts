...
describe('OT-72 provider truth migration and repository', () => {
  it('applies the 1700 namespace and stores only redacted provider truth', async () => {
    const pool = createMemoryPool();
    try {
      await runMigrations(pool);
      const applied = await pool.query(
        "SELECT checksum FROM onetime.schema_migrations WHERE id = '1700_ot72_provider_truth'",
      );
