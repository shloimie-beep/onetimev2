  ...
  it('renders dry-run metrics and prepared segments without raw row values', () => {
    const markup = renderToStaticMarkup(
      React.createElement(AudienceReconciliationPanel, {
        ...
        taxonomyFacts: legacyAudienceGovernedTaxonomy,
      }),
    );
    ...
    expect(markup).toContain('Governed Facts');
    expect(markup).toContain('Campaign candidate');
...
function summary(): LegacyAudienceSummary {
  return {
    ...
    taxonomy_fact_counts: {
      campaign_candidate: 1,
      contact_person: 3,
      do_not_send: 1,
      manual_review: 1,
    },
