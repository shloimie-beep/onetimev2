...
test('CRM shell emits post-paint marks and meets 30-sample performance gates', async ({
  ...
  const requestCounts = {
    ...
    warm_return_list_requests: [] as number[],
  ...
  const report = {
    profile: {
      ...
      measurement_definition:
        'List/detail marks are recorded from React effects only after committed visible/actionable UI and at least the next animation frame have painted. Warm return starts at Back to CRM click and must reuse the in-memory list without a full list request.',
  ...
  expect(report.request_counts.warm_return_list_requests.every((count) => count === 0)).toBe(true);
...
async function measureList(page: Page) {
  ...
  return {
    ...
    request_count: requests.paths.filter(
      (requestPath) => requestPath === '/api/v1/crm/contacts/search',
    ).length,
